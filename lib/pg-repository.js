...
exports.Repository = repository_1.generate(class {
    constructor(dbUrl) {
        this.extensions = {
            authenticateAsync: (credentials) => __awaiter(this, void 0, void 0, function* () {
                const dbUsers = yield this.getAsync({
                    entity: "users",
                    parentId: "",
                ...
                return new Promise(resolve => {
                    bcrypt.compare(credentials.pwd, dbUser.pwdHash, (error, result) => {
                        if (result === true) {
                            resolve(dbUser);
                        }
                        else {
            ...
            registerAsync: (args) => __awaiter(this, void 0, void 0, function* () {
                const { password } = args, user = __rest(args, ["password"]);
                const salt = bcrypt.genSaltSync();
                const pwdHash = bcrypt.hashSync(password, salt);
                const userToBeRegistered = Object.assign(Object.assign({}, user), { pwdHash: pwdHash, pwdSalt: salt });
                return yield this.saveAsync({ entity: "users", obj: userToBeRegistered, mode: "insert" });
            }),
            unregisterAsync: (id) => __awaiter(this, void 0, void 0, function* () { return this.deleteAsync({ entity: "users", id }); }),
            findUserAsync: (userid) => __awaiter(this, void 0, void 0, function* () { return this.findAsync({ entity: "users", id: userid }); }),
            //getUsersAsync: async (role: User["role"]) => this.getAsync("user", { role }),
            updateUserAsync: (obj) => __awaiter(this, void 0, void 0, function* () { return this.saveAsync({ entity: "users", obj, mode: "update" }); }),
            insertResultsAsync: (results) => __awaiter(this, void 0, void 0, function* () { throw new Error(`insertResultsAsync not implemented`); }),
            deleteResultsAsync: (analysisId) => __awaiter(this, void 0, void 0, function* () {
            })
        };
    ...
    saveAsync(args) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (args.mode === "insert") {
                ...
                const columns = keys.map(k => new stdlib_1.String__(k.toString()).toSnakeCase()).join(",");
                ...
                let assignmentsClause = keys
                    ...
                    .map((key, index) => `${new stdlib_1.String__(key.toString()).toSnakeCase()} = $${index + 1}`).join(', ');
...
function getTableName(entityName) {
    const plural = new stdlib_1.String__(entityName).plural();
    return plural.toLocaleLowerCase();
}
function getColumnName(propertyName) {
    return new stdlib_1.String__(propertyName).toSnakeCase().toLocaleLowerCase();
}
...
function dbToApp(entity, serverObj) {
    ...
    Object.keys(serverObj).forEach(key => {
        const typedKey = new stdlib_1.String__(key).toCamelCase().toString();
