...
export function asQueryParams<T extends Obj = Obj>(obj: T, excludedValues: any[] = [undefined, null]) {
	var str = []
	for (var p in obj)
		if (obj.hasOwnProperty(p) && !excludedValues.includes(obj[p])) {
...
export async function getAsync(request: GetRequest) {
	return new Promise<Request.Response>((resolve, reject) => {
		...
		Request.get({
			uri: `${trimRight(request.uri, "/")}${queryParams}`,
...
function trimRight(str, ...strings: string[]) {
