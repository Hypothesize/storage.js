...
import { Obj, Filter, FilterGroup, keys, hasValue } from "@sparkwave/standard"
...
export interface DbProvider {
	...
	select<F extends Filter<Obj<DbPrimitive>> | FilterGroup>(rowsetName: string, filters?: F): string
}
...
export abstract class PostgresDbProvider implements DbProvider {
	...
	protected predicateTemplates(): Obj<undefined | ((x: DbPrimitive) => string), Required<Filter>["operator"]> {
		return {
			equal: x => hasValue(x) && x !== null ? `= ${this.interpolatableValue(x)}` : `is NULL`,
			not_equal: x => hasValue(x) && x !== null ? `<> ${this.interpolatableValue(x)}` : `is not NULL`,
			greater: x => hasValue(x) && x !== null ? `> ${this.interpolatableValue(x)}` : `> NULL`,
			less: x => hasValue(x) && x !== null ? `< ${this.interpolatableValue(x)}` : `< NULL`,
			greater_or_equal: x => hasValue(x) && x !== null ? `>= ${this.interpolatableValue(x)}` : `>= NULL`,
			less_or_equal: x => hasValue(x) && x !== null ? `<= ${this.interpolatableValue(x)}` : `<= NULL`,
	...
	select(rowsetName: string, filter?: Filter<Obj<DbPrimitive>> | FilterGroup): string {
		return `SELECT * FROM ${this.interpolatableRowsetName(rowsetName)}() WHERE ${this.getWhereClause(filter)}`
	}

	getWhereClause(filter?: Filter<Obj<DbPrimitive>> | FilterGroup): string {
